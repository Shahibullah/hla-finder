...
@section('title', 'Receiver Transplant History')
...
">
    ...
    ">
        ...
        <p style="
            ...
            margin-bottom: 25px;
        ">
            View your personal transplant records here.
        </p>
                    ...
                    <tbody>
                        @foreach($histories as $history)
                            <tr style="background: {{ $tableRowBg }};">
                                ...
                                <td style="padding: 12px; border: 1px solid {{ $borderColor }};">{{ $history->donor->name ?? 'N/A' }}</td>
                                <td style="padding: 12px; border: 1px solid {{ $borderColor }};">{{ $history->lab->name ?? 'N/A' }}</td>
                                <td style="padding: 12px; border: 1px solid {{ $borderColor }};">{{ $history->transplant_date }}</td>
                                <td style="padding: 12px; border: 1px solid {{ $borderColor }};">{{ $history->organ_type }}</td>
                                <td style="padding: 12px; border: 1px solid {{ $borderColor }};">{{ $history->outcome }}</td>
                                <td style="padding: 12px; border: 1px solid {{ $borderColor }};">{{ $history->condition_notes ?? 'N/A' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
